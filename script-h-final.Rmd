...
---
title: 'Heritability and spatial distribution of aphid mummies on wheat ears'
...
## Phenotypic data
Phenotypic data is loaded
```{r}
setwd("C:/Users/capuc/Desktop/JRL 2021/script/SCRIPT.JRL") # modify according to the directory place

A<-read.table("sortie2.csv", sep=";", header = T)
...
hist(A$variable, main=paste("Distribution of", variable1), cex.main=0.9, xlab=variable1, col="pink")
```
...
zp1 <- ggplot(A, aes(x =planche, y =passage_inverse, fill = variable))
zp1 <- zp1 + geom_tile()
zp1 <- zp1 + coord_equal()
zp1
...
which(BLUP>0.8)
BLUE[118]
hist(BLUE, col="yellow")
hist(BLUP, col="orange")
...
## Heritability
Heritabilities with and without correction are estimated
...
#mnaif
VG/(VG+VE)#individual heritability
VG #genotypic variance
VE #environmental variance
```

Design heritability (mean of the two genotypes)
```{r}
VG/(VG+VE/2)
...
Creation of a covariable to see links between number of mummies independantly from the genotypes of the wheat. 
The function obtain.spatialtrend is used. 
